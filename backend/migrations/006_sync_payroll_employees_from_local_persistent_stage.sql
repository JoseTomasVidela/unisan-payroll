...
-- Version compatible con DBeaver: usa tabla staging persistente.
...
DROP TABLE IF EXISTS payroll_employee_sync_stage_perm;
CREATE TABLE payroll_employee_sync_stage_perm (
...
INSERT INTO payroll_employee_sync_stage_perm (
...
JOIN payroll_employee_sync_stage_perm AS src
...
FROM payroll_employee_sync_stage_perm AS src
...
SELECT COUNT(*) AS staged_workers FROM payroll_employee_sync_stage_perm;
SELECT COUNT(*) AS total_workers_after_sync FROM payroll_employees;
SELECT COUNT(*) AS workers_with_rut_after_sync FROM payroll_employees WHERE rut IS NOT NULL AND TRIM(rut) <> ''; 

-- Opcional: descomenta esta linea si quieres limpiar la staging al finalizar.
-- DROP TABLE payroll_employee_sync_stage_perm;
