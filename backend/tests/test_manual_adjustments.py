...
def test_admin_can_edit_manual_adjustment_without_description(client, db_factory):
    ...
    assert updated.status_code == 200
    body = updated.json()
    assert body["adjustment_type"] == "DISCOUNT"
    assert body["description"] == "Descuento"
...
def test_manual_adjustment_list_returns_history_and_audit_rows(client, db_factory):
    ...
    assert len(listed) == 1
    assert listed[0]["description"] == "Vacaciones actualizadas"
    ...
    with db_factory() as db:
        ...
        audits = db.scalars(
            ...
            .where(PayrollAuditLog.record_id == adjustment_id)
        ).all()
        assert len(audits) == 2
